...
const getAssessments = (coursePage) => {
  ...
  while (assessmentMatch) {
    ...
    assessment.name = filterSpecialSymbols(
      nameRegex.exec(assessmentStr)[1].trim(),
    );
    ...
    Object.keys(strandColours).forEach((strand) => {
      ...
      const strandWeightMatch = /(weight=(.*?)|(no weight))</.exec(
        assessmentStr,
      );
      if (strandMarkMatch) {
...
const updateDB = async (courses, userId) => {
  ...
  const asyncFunctions = courses.map((course) => async () => {
    ...
    try {
      ...
      const assessmentNames = course.assessments.map(
...
const updateCourseAvg = async (course, userId) => {
  const courseOverviewCollection = context.services
    .get('tsapp-service')
    .db('tsapp')
  ...
  assessments.forEach((assessment) => {
    Object.keys(assessment.marks).forEach((strand) => {
      if (
        ...
        marks[strand][0] +=
          (mark[strand].numerator / mark[strand].denominator) *
          mark[strand].weight;
...
const updateAssessments = async (course, courseId, userId) => {
  const assessmentCollection = context.services
    .get('tsapp-service')
    .db('tsapp')
  ...
  const taAssessments = course.assessments.map((assessment, index) => [
    assessment,
    index,
  ]);
  ...
  await Promise.all(
    assessments.map(async (assessmentDoc) => {
      const dbAssessment = {
      ...
      const assessmentIdx =
        match.length > 0 ? taAssessments.indexOf(match[0]) : -1;

      //if assessment exists in db but not ta
      if (assessmentIdx === -1) {
        console.log('deleted assessment');
        modified = true;
        return assessmentCollection.findOneAndDelete({
  ...
  await Promise.all(
    taAssessments.map(async (taAssessment) => {
...
const updateCourseOverviews = async (userId, courseOverviews) => {
  ...
  const courseOverviewCollection = context.services
    ...
    .db('tsapp')
    .collection('courseOverviews');
  const courseOverviewsDb = await courseOverviewCollection
    .find({
      date: now,
      userId: userId,
    })
  ...
  const asyncFunc = courseOverviewsDb.map((overview) => async () => {
    ...
    const sameCourseIdx =
      match.length > 0 ? courseOverviews.indexOf(match[0]) : -1;

    //user no longer registered in this course
    //dropped out or smth
    if (sameCourseIdx === -1) {
      courseOverviewCollection
        .findOneAndDelete({
          _id: overview._id,
        })
        .catch(console.log);
  ...
  if (courseOverviews.length > 0) {
    ...
    return Promise.all(
      courseOverviews.map(async (taCourseOverview) => {
        return courseOverviewCollection.insertOne({
          date: now,
          userId: userId,
          ...taCourseOverview,
